...
import { fetchcollectionfilteredProducts, fetchcollectionMorefiltered, fetchfilteredProducts, fetchMorefiltered } from '@/lib/reducers/productReducer'
import { fetchCategories } from '@/lib/reducers/categoryReducer'
import Link from 'next/link'
import CatlogPriceFilter from '../../categories/[...category]/myslider'
    ...
    const  filters = [
      {
        ...
        name: 'Collection',
        multiple: true,
        options: collections.filter((e)=>e.value !== getcollection)
        ,
      },
      {
        id: 'shopFor',
...
const {loadingProductId} = useSelector((state)=>state.cart)
  ...
  const fetchmoreData = ()=>{
    ...
    dispatch(fetchcollectionMorefiltered({pathsparams:getcollection,query:{...data, pageNumber:data.pageNumber+1}}))
  }
 ...
 const handleAddToCart = async (product) => {
  ...
  useEffect(()=>{
    ...
    setSelectedFilters({collection,minPrice,maxPrice,shopFor,category})
    setData(parameter);
    console.log(parameter)
    dispatch(fetchcollectionfilteredProducts({pathsparams:getcollection,query:parameter}))
  ...
  return (
    <div className="bg-white">
      ...
      {categories.length ? <Disclosure key="category" as="div" className="border-b pl-2 border-gray-200 py-6">
          <h3 className="-my-3 flow-root">
            <DisclosureButton className="group flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500">
              ...
              {categories?.map((option, idx) => (
                <div key={option.name} className="flex gap-3">
                  <input
                    type="checkbox"
                    className="w-5 h-5 star appearance-none cursor-pointer border text-blue-gray-600 border-gray-300  rounded-md mr-2 hover:border-indigo-500 hover:bg-indigo-100 checked:bg-no-repeat checked:bg-center checked:border-indigo-500 checked:bg-indigo-100"
                    id={`filter-collection-${idx}`}
                    name={`category[]`}
                    value={option.slug}
                    checked={selectedFilters["category"]?.split(',').includes(option.slug)}
                    onChange={() => handleFilterChange("category", option.slug, true)}
                  />
           
                  <label htmlFor={`filter-category-${idx}`} className="cursor-pointer ml-2 text-slate-600  text-sm text-gray-600">
                    {option.name}
                  </label>
                </div>
              ))}
  ...
  <CatlogPriceFilter sliderMinValue={priceRange.minPrice} sliderMaxValue={priceRange.maxPrice}  minVal={selectedFilters.minPrice} maxVal={selectedFilters.maxPrice} setMaxVal={setMaxPrice} setMinVal={setMinPrice}/> 
      ...
      <button type='button' onClick={ResetPrice} className=' bg-gray-500 py-1 px-3 rounded-md hover:text-white hover:bg-red-300' >Reset Price</button>
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 capitalize">{getcollection}</h1>
