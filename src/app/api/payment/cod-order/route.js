...
function generateEmailContent(orders) {
  ...
  return `
      ...
      <body>
          ...
          ${orderDetails}
          <a href="https://yourtrackinglink.com" style="display: inline-block; background-color: #181818; color: #F8F8F8; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Track Your Order</a>
      </body>
      </html>
  `;
}
export async function POST(req) {
  ...
  try {
    ...
    console.log(address,address.country.label)
    let Id;
    if(!userId) {
     Id = await UserAuth();
  }else{
